		...
		"bootstrap": {
			"fnInit": function( oSettings, nPaging, fnDraw ) {
				...
				$(nPaging).append(
					'<ul class="pagination">'+
						...
						'<li class="redirectli disabled"><a href="javascript:;" style="padding:4px 12px 4px 12px;"><input type="text" style="width:30px;margin:0;padding:0;text-align:center;" id="redirect" class="redirect"></a></li>'+//跳转指定页码
					'</ul>'
				);
				 //datatables分页跳转
				...
				//$(els[4]).bind( 'click.DT', { action: "goto" }, fnClickHandler );//指定页
			...
			"fnUpdate": function ( oSettings, fnDraw ) {
				...
				for ( i=0, ien=an.length ; i<ien ; i++ ) {
					...
					$('li:gt(0)', an[i]).filter(':not(.next,.prev,.redirectli)').remove();//ACE
